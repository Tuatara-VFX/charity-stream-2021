...
void SampleCameraColor_float(float2 UV, out float4 Color)
{
    #if SHADERGRAPH_PREVIEW == 1
    Color = float4(0, 0, 0, 1);
    #else
    Color = float4(SampleCameraColor(UV), 1.0);
    #endif
}

void SampleCameraColor_half(half2 UV, out half4 Color)
{
    #if SHADERGRAPH_PREVIEW == 1
    Color = half4(0, 0, 0, 1);
    #else
    Color = half4(SampleCameraColor(UV), 1.0);
    #endif
}

void SampleCameraDepth_float(float2 UV, out float Depth)
{
    #if SHADERGRAPH_PREVIEW == 1
    Depth = 0;
    #else
    Depth = LinearEyeDepth(SampleCameraDepth(UV), _ZBufferParams);
    #endif
}

void SampleCameraDepth_half(half2 UV, out half Depth)
{
    #if SHADERGRAPH_PREVIEW == 1
    Depth = 0;
    #else
